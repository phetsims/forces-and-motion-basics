...
/**
 * Shared helpers for deriving localized and accessible descriptions for Motion screen items.
 * Provides a centralized class so that ItemNode, MotionStackDescriptionNode, and other clients
 * can share the same logic without duplicating utility functions.
 *
...
export default class ItemDescriber {
   ...
   */
  public static getLocalizedItemNameProperty( item: Item ): TReadOnlyProperty<string> {
   ...
   */
  public static createItemAccessibleNameWithMassProperty( item: Item ): TReadOnlyProperty<string> {
   ...
   */
  public static formatItemAccessibleNameWithMass( item: Item ): string {
    const itemName = ItemDescriber.getLocalizedItemNameProperty( item ).value;
  ...
  /**
   * Returns the contextual announcement string when an item is hovered over the stack area.
   */
  public static getOverAreaMessageForStackHover( model: MotionModel ): string {
    const hasOtherItems = model.stackedItems.length > 0;
    if ( hasOtherItems ) {
  ...
  /**
   * Returns the contextual response Property to announce after an item is dropped on the stack.
   */
  public static getDroppedOnStackResponse( model: MotionModel, priorLength: number ): TReadOnlyProperty<string> {
    if ( priorLength === 0 ) {
      return model.screen === 'motion' ?
      ...
      return ForcesAndMotionBasicsFluent.a11y.motionScreen.objectResponses.droppedOnStackStringProperty;
    }
    else {
      return ForcesAndMotionBasicsFluent.a11y.motionScreen.objectResponses.droppedOnStackBottomObjectReturnedStringProperty;
    }
  }
}

forcesAndMotionBasics.register( 'ItemDescriber', ItemDescriber );
